...
<!--<div id="imgLoL"><img src="img/lol.png" alt="lol logo"/></div>-->
<div id="main-div">
<div id="twentyfive"></div>
<div id="form-div">
<form class="form" id="form1" method="get">
            <p class="summoner">
                <input name="summoner" type="text" placeholder="Summoner's Name" class="feedback-input" id="summoner"/>
            </p>
                <!--<input type="submit" formaction="displayElo.php" name="submit" value="CLR BROWSER ONLY"/>-->
                <div class="submit">
                <input type="submit" name="submit" value="Go" id="button-blue" onclick="displayElo();"/>
                <input type="submit" formaction="displayElo.php" name="submit" value="CLR ELO" id="buttonblue2"/>
                <input type="submit" formaction="masteries.php" name="submit" value="CLR MASTERIES" id="buttonblue3"/>
                <div class="ease"></div>
                </div>
                <!--<input type="submit" formaction="masteries.php" name="submit" value="MASTERIES TAB ON CLR BROWSER"/>-->
</form>
</div>

<script>
function displayElo(){
	document.getElementById("dispElo").innerHTML = "<?php
$Go = $_GET['submit'];
if($Go == 'Go'){
	?></script><?php
}
...
$masteryLocal = $api->getStaticLocal('mastery');
...
?>
<div id="leagueInfo2">
	<p  class="username"><?php echo($_GET['summoner']);?></p>
	<?php
	if ($tier == 'DIAMOND'){
	...
	<div id="statsLeague">
		...
		<p class="lp"><?php echo($nbLP." LP");?>
			...
			if(end($miniSeries)){
				...
				?></p>
				<p class="winlosses"><span class="vic"><?php echo($wins. " Wins")?></span> -- <span class="def"><?php echo($losses." Losses");?></span></p>
	</div>
</div>
</div><?php
try {
...
?><!--<div id="runesInfo"><?php
$x = 0;
while($x < sizeof($currentUserInGame['runes'])){
	$rune[$x] = $currentUserInGame['runes'][$x];
	$countRune[$x] = ($rune[$x]['count']);
	$runeId[$x] = ($rune[$x]['runeId']);
	$runeStatic  = $api->getStatic('rune',$runeId[$x]);
	$runeStatRaw = reset($runeStatic['stats']);
	echo($runeStatic['name']);
	echo(" : ");
	echo($countRune[$x]." x ".$runeStatic['description']." = ".$runeStatRaw*$countRune[$x]);
	echo("</br>");
	echo("</br>");
	$x++;
}
